<template>
  ...
  >
    <button
      ...
      :style="cell.exploded ? { animationDelay: `${(cell.index % 5) * 60}ms` } : null"
      @click="onPick(cell.index)"
    >
      ...
      <template v-if="mode === 'select'">
        <span
          ...
          :class="cell.index === mySquare ? 'text-white' : 'text-slate-200'"
        >
          {{ cell.label }}
        </span>
        <span
          v-if="cell.index === mySquare"
          class="absolute right-1 top-1 text-sm"
        >
          ✓
        </span>
      </template>
      ...
      <template v-else>
        ...
        >
        <span
          v-else-if="cell.tiebreakLoser"
          class="absolute -top-2 right-0 text-lg drop-shadow"
          >⏱️</span
        >
        <span
          v-if="cell.label"
        ...
        <div class="absolute inset-0 flex flex-wrap content-center items-center justify-center gap-0.5 p-1">
          <span
            ...
            :class="cell.exploded ? 'bg-rose-200 text-rose-900' : cell.tiebreakLoser ? 'bg-sky-200 text-sky-900' : 'bg-emerald-200 text-emerald-900'"
            :title="name"
          >
            {{ initials(name) }}
          </span>
...
function cellClass(cell) {
  ...
  // reveal
  if (cell.exploded) {
    return 'bg-rose-500/25 ring-rose-400/60 animate-explode'
  }
  if (cell.tiebreakLoser) {
    return 'bg-sky-500/20 ring-sky-400/50'
  }
  if (cell.occupants.length === 1) {
    return 'bg-emerald-500/20 ring-emerald-400/50'
  }
  return 'bg-white/[0.03] ring-white/5'
}

function onPick(index) {
  if (props.mode === 'select' && !props.disabled) emit('pick', index)
}
